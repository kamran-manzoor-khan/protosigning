...
<div class="d-flex" id="root" ref="root" :style="{height: root['h']+'px'}">
    ...
    <div class="p-1 m-0" ref="right" :style="{width: right['w']+'px', height: right['h']+'px'}">
        ...
        <div class="p-2 transition-3 w-100 d-flex align-items-center justify-content-center overflow-auto" ref="workarea" :style="{height: workarea['h'] + 'px'}">
            <div id="svg-container" :style="{width: canvas.w + 'px', height: canvas.h + 'px'}" class="border bd-2 p-relative hov-elem" @mouseleave="more_vert.canvas.dropdown = false">
                <!-- PROPS -->
    ...
    <div v-if="more_vert.selector.show" class="p-absolute animated zoomIn faster" :style="more_vert.selector.style">
        <button @click="more_vert.selector.dropdown = !more_vert.selector.dropdown" class="btn text-white p-1 box-shadow bd-round material-icons bg-primary-2">more_vert</button>
...
export default {
    ...
    data() {
        return {
            ...
            canvas: { node: null, $: null, w: 800, h: 800 },
            more_vert: {
                canvas: {
                    $: null,
                    list: [],
    ...
    mounted() {
        ...
        // $(window).mousemove((e) => this.ev._move(e))
